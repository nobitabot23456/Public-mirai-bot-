...
export const config = {
    ...
    description: "Send an anime dancing GIF",
...
export async function run({ api, event, message }: any) {
    ...
    try {
        const { url, animeTitle } = await fetchNekosBest("dance");
        const caption = `💃 vibing~${animeTitle ? `\n📺 ${animeTitle}` : ""}`;
        await sendImageFromURL(api, event, url, caption);
        await message.reaction("✅");
    } catch {
        await message.reaction("❌");
        message.reply("Failed to fetch dance GIF.");
    }
}
