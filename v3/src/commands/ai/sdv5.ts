...
export const config = {
    ...
    cooldowns: 5,
    hasPermission: 0,
    description: "edit & generate images using SeeDream V5",
...
export async function run({ message, event, args }: any) {
    ...
    if (!reply || reply.attachments.length === 0) {
        ...
        try {
            const gres = await axios.get(`https://tawsif.is-a.dev/seedream-v5/gen?prompt=${encodeURIComponent(prompt)}&ratio=${encodeURIComponent(ratio)}`);
            await message.reply({
                body: "✅ | Generated",
        ...
        await message.reaction("⏳");
        try {
            const eres = await axios.get(`https://tawsif.is-a.dev/seedream-v5/edit?prompt=${encodeURIComponent(prompt)}&urls=${encodeURIComponent(JSON.stringify(imgs))}`);
            await message.reply({
