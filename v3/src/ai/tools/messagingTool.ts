...
export const sendMessageTool = tool(
  withToolMiddleware(async ({ text }: { text: string }, config: any) => {
    ...
    return new Promise((resolve) => {
      api.sendMessage(text, event.threadID, (err: any, info: any) => {
        if (err) {
          resolve(`Failed to send message: ${JSON.stringify(err)}`);
        } else {
          resolve(`Message sent successfully. ID: ${info.messageID}`);
        }
      }, event.messageID);
    });
  }, "sendMessage"),
...
export const unsendTool = tool(
  withToolMiddleware(async ({ messageID }: { messageID: string }, config: any) => {
    ...
    return new Promise((resolve) => {
      api.unsendMessage(messageID, (err: any) => {
        if (err) {
          resolve(`Failed to unsend: ${JSON.stringify(err)}`);
        } else {
          resolve("Message unsent successfully.");
        }
      });
    });
  }, "unsend"),
